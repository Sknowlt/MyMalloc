...
{	
	void * blockToFree = bp;
	PUT(HDRP(blockToFree), PACK(GET_SIZE(HDRP(blockToFree)), 1));	
	char * previousBlock = heap_listp;
	char * currentBlock = NEXT_BLKP(heap_listp);
	while(GET_SIZE(HDRP(currentBlock)) != 0){
		if(GET_ALLOC(HDRP(previousBlock)) == 1 && GET_ALLOC(HDRP(currentBlock)) == 1){
			PUT(HDRP(previousBlock), PACK(GET_SIZE(HDRP(previousBlock)) + GET_SIZE(HDRP(currentBlock)), 1));
			currentBlock = NEXT_BLKP(currentBlock);
		} else {
			previousBlock = NEXT_BLKP(previousBlock);
			currentBlock = NEXT_BLKP(currentBlock);	
		}
	}
}
...
void *mm_realloc(void *ptr, size_t size)
{
	if(ptr == NULL){
		...
		// size_t pointerSize = GET_SIZE(HDRP(NEXT_BLKP(ptr))) - size;
		// //printf("Pointer Size: %d\n", pointerSize);
		// //printf("Input Size: %d\n", size);
		// if(GET_ALLOC(HDRP(NEXT_BLKP(ptr))) == 1 && (GET_SIZE(HDRP(ptr))+GET_SIZE(HDRP(NEXT_BLKP(ptr)))) > size){
		// 	//printf("Entering IF statement!\n");
		// 	if((pointerSize & 0x7) == 0){
		// 		//printf("0x7 != 0!\n");
		// 		PUT(HDRP(ptr), PACK(GET_SIZE(HDRP(ptr)) + size, 0));
		// 		PUT(HDRP(NEXT_BLKP(ptr)), PACK(GET_SIZE(HDRP(NEXT_BLKP(ptr))) - size, 0));
		// 	} else if((pointerSize & 0x7) != 0){
		// 		//printf("0x7 == 0!\n");
		// 		PUT(HDRP(ptr), PACK(GET_SIZE(HDRP(NEXT_BLKP(ptr))) + GET_SIZE(HDRP(ptr)), 0));
		// 	}
		// 	return ptr;
		// }
